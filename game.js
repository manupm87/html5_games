...
function RunGameFrame(Cars)
{
    ...
    for (var i = 0; i < Cars.length; i++)
    {
        ...
        if (Cars[i].X <= GP.CarRadius/10 || Cars[i].X >= GP.GameWidth - GP.CarRadius/10) {
            ...
            if ((Cars[i].X <= GP.CarRadius/10 && Cars[i].VX <= 0)
                || (Cars[i].X >= GP.GameWidth - GP.CarRadius/10 && Cars[i].VX >= 0)) {
                //Impulses.push([i, null, 2 * Cars[i].VX, 0]); // Turn the car around. 
                
                // Left and right walls are death! -> reborn in the middle
                Cars[i].X = GP.GameWidth / 2;
                Cars[i].Y = GP.GameHeight*Math.random();
                Cars[i].VX = 0;
                Cars[i].VY = 0;
                Cars[i].OR = 0;
            }
        ...
        for (var j = i + 1; j < Cars.length; j++) {
            ...
            if (Math.sqrt(DistSqr) <= 2 * GP.CarRadius) {
                // The impulses from a two dimensional 
                // Delta = (r_j - r_i) . (v_i - v_j) / elastic collision. |r_j - r_i|^2.
